    ...
    for i in range(0, len(dataset), batch_size):
        ...
        if i == 0:  # Log first batch details
            ...
            log(
                logging.INFO,
                "Single batch memory:"
                f" {batch_data.element_size() * batch_data.nelement() / 1024 / 1024:.2f}MB",
            )
...
def test_federated_dataloader(
    ...
    print(f"loading test set from: {partition_dir}")
...
def download_and_preprocess(cfg: DictConfig) -> None:
    ...
    for idx in range(cfg.dataset.num_clients):
        ...
        # log(logging.INFO, f"Processing client_{idx}: Train data {train_data.shape}, Train targets {train_targets.shape}")

        # Log detailed size information
        # log(logging.INFO, f"Client {idx} data shapes:")
        # log(logging.INFO, f"  Train data: {train_data.shape}, dtype={train_data.dtype}")
        # log(logging.INFO, f"  Train targets: {train_targets.shape}, dtype={train_targets.dtype}")
        # log(logging.INFO, f"  Memory usage (train): {train_data.element_size() * train_data.nelement() / 1024 / 1024:.2f}MB")
